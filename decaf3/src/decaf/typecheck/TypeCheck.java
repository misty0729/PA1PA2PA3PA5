...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitScopy(Tree.Scopy scopy) {
    	...
    	if(scopy.ident.type.equal(BaseType.ERROR) || scopy.expr.type.equal(BaseType.ERROR)) {
    		scopy.type = BaseType.ERROR;
    		return;
    	}
    	if(scopy.ident.type.isClassType()) {
    		if(scopy.ident.type.equal(scopy.expr.type))
    			scopy.type = scopy.ident.type;
    		else {
    			issueError(new BadScopySrcError(scopy.getLocation(),
						scopy.ident.type.toString(),scopy.expr.type.toString()));
				scopy.type = BaseType.ERROR;
    		}
    	}
    	else {
    		issueError(new BadScopyArgError(scopy.getLocation(),"dst",
    				scopy.ident.type.toString()));
    		scopy.type = BaseType.ERROR;
	...
	public void visitIfSubStmt(Tree.IfSubStmt ifsubstmt) {
		...
		if (ifsubstmt.condition.type!=BaseType.BOOL) {
			issueError(new BadTestExpr(ifsubstmt.getLocation()));
			ifsubstmt.type = BaseType.ERROR;
			return;
		}
		ifsubstmt.stmt.accept(this);
		ifsubstmt.type = BaseType.BOOL;
	}
	
	@Override
	public void visitGuardedStmt(Tree.GuardedStmt guardedstmt) {
		for(Tree f : guardedstmt.slist) {
			f.accept(this);
			if(f.type == BaseType.ERROR) 
				guardedstmt.type = BaseType.ERROR;
		}
	}
	@Override
	public void visitVarStmt(Tree.VarStmt varstmt) {
		varstmt.type = BaseType.UNKNOWN;
//		Symbol v = table.lookupBeforeLocation(varstmt.ident, varstmt
//				.getLocation());
//		if (v == null) {
//			issueError(new UndeclVarError(varstmt.getLocation(), varstmt.ident));
//			varstmt.type = BaseType.ERROR;
//		} else if (v.isVariable()) {
//			Variable var = (Variable) v;
//			if (var.isLocalVar()) {
//				varstmt.lvKind = Tree.LValue.Kind.LOCAL_VAR;
//			} else if (var.isParam()) {
//				varstmt.lvKind = Tree.LValue.Kind.PARAM_VAR;
//			} else {
//				varstmt.lvKind = Tree.LValue.Kind.MEMBER_VAR;
//			}
//		}
//		Variable v = new Variable(varstmt.ident, varstmt.type, 
//				varstmt.getLocation());
//		Symbol sym = table.lookup(varstmt.ident, true);
//		if (sym != null) {
...
//		System.out.println(assign.expr.type);
		if(assign.left.type.equal(BaseType.UNKNOWN)) {
			if(!assign.expr.type.equal(BaseType.ERROR)) {
				...
				Symbol sym = table.lookup(((Tree.VarStmt)assign.left).ident, true);
				table.cancle(sym);
				Variable v = new Variable(((Tree.VarStmt)assign.left).ident, assign.left.type, 
						assign.left.getLocation());
				((Tree.VarStmt)assign.left).symbol = v;
				table.declare(v);
//				if (v.isLocalVar()) {
					assign.left.lvKind = Tree.LValue.Kind.LOCAL_VAR;
//				} else if (v.isParam()) {
//					assign.left.lvKind = Tree.LValue.Kind.PARAM_VAR;
//				} else {
//					assign.left.lvKind = Tree.LValue.Kind.MEMBER_VAR;
//				}
...
//		System.out.println(initarray.right.type);
		if(initarray.left.type.equal(BaseType.ERROR) ||initarray.right.type.equal(BaseType.ERROR) ) {
			initarray.type=BaseType.ERROR;
					return;
		}
//		System.out.println(initarray.left.type);
		if(initarray.left.type.equal(BaseType.UNKNOWN) || initarray.left.type.equal(BaseType.VOID)) {
			issueError(new BadArrElementError(initarray.getLocation()));
			initarray.type = BaseType.ERROR;
//			System.out.println(initarray.type);
		}
		if(!initarray.right.type.equal(BaseType.INT)) {
			issueError(new BadArrIndexError(initarray.getLocation()));
			initarray.type=BaseType.ERROR;
			return;
		}
		if(initarray.type == null)
			initarray.type=new ArrayType(initarray.left.type);
	}
	...
	public void visitDefault(Tree.Default mydefault) {
		...
		if(mydefault.left.type.equal(BaseType.ERROR) ||mydefault.right.type.equal(BaseType.ERROR)||mydefault.arrayname.type.equal(BaseType.ERROR) ) {
			mydefault.type=BaseType.ERROR;
			return;
		}
		if(!mydefault.left.type.equal(BaseType.INT)) { 
			issueError(new BadArrIndexError(mydefault.getLocation()));
		}
		if(mydefault.arrayname.type.isArrayType()) { // if it is a arraytype
//			System.out.println("yes");
			mydefault.type =  ((ArrayType)mydefault.arrayname.type).getElementType();
			if(!mydefault.right.type.equal(mydefault.type)) {
				issueError(new BadDefError(mydefault.getLocation(),mydefault.type.toString(),mydefault.right.type.toString()));
			}
		}
		else {
			issueError(new BadArrOperArgError(mydefault.getLocation()));
			if(mydefault.right.type.equal(BaseType.UNKNOWN) || mydefault.right.type.equal(BaseType.VOID)) {
//				issueError(new BadArrElementError(mydefault.getLocation()));
				// something todo
				mydefault.type = BaseType.ERROR;
			}
			else {
				mydefault.type = mydefault.right.type;
			}
		}
	}
	...
	public void visitForeachStmt(Tree.ForeachStmt foreach) {
		...
		foreach.lvalue.accept(this);
		if(foreach.expr1.type.equal(BaseType.ERROR)) {
			return;
		}
		if(((Tree.NormalStmt)foreach.lvalue).literaltype.type.equal(BaseType.UNKNOWN)) { // if we need to get the type
		if(foreach.expr1.type.isArrayType()) {
			
			((Tree.NormalStmt)foreach.lvalue).literaltype.type= ((ArrayType)foreach.expr1.type).getElementType();
			 // something to do, not judge if there is another var has the same name
//			Symbol sym = table.lookup(((Tree.VarStmt)foreach.lvalue).ident, true);
//			table.cancle(sym);
			Variable v = new Variable(((Tree.NormalStmt)foreach.lvalue).ident, ((Tree.NormalStmt)foreach.lvalue).literaltype.type, 
					foreach.lvalue.getLocation());
			((Tree.NormalStmt)foreach.lvalue).symbol = v;
			table.declare(v);
		}
		else {// if it is not a arraytype
//			System.out.println("i did it!");
			issueError(new BadArrOperArgError(foreach.getLocation()));
			((Tree.NormalStmt)foreach.lvalue).literaltype.type = BaseType.ERROR;
			foreach.type = BaseType.ERROR;
		}
	}
	else {
//		System.out.println(foreach.expr1.type);
		if(!foreach.expr1.type.isArrayType()) {
			issueError(new BadArrOperArgError(foreach.getLocation()));
			((Tree.NormalStmt)foreach.lvalue).literaltype.type = BaseType.ERROR;
		}
		else {
		if(!((ArrayType)foreach.expr1.type).getElementType().compatible(((Tree.NormalStmt)foreach.lvalue).literaltype.type)) {
			// if it is not compatibale
			issueError(new BadForeachTypeError(foreach.getLocation(),((ArrayType)foreach.expr1.type).getElementType().toString(),((Tree.NormalStmt)foreach.lvalue).literaltype.type.toString()));
		}
		}
//		Variable v = new Variable(((Tree.NormalStmt)foreach.lvalue).ident, ((Tree.NormalStmt)foreach.lvalue).literaltype.type, 
//				foreach.lvalue.getLocation());
//		table.declare(v);
		...
		if(foreach.expr2 != null) {
			...
			if(!foreach.expr2.type.equal(BaseType.BOOL)) {
				issueError(new BadTestExpr(foreach.getLocation()));
			}
			if(foreach.expr2.type.equal(BaseType.ERROR))
				return;
		}
		if(foreach.type != BaseType.ERROR) {
			breaks.add(foreach);
		if(foreach.stmt instanceof Tree.Block) {
			for (Tree s : ((Tree.Block)foreach.stmt).block) {
				s.accept(this);
			}
		}
		else {

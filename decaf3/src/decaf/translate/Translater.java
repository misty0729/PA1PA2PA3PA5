...
public class Translater {
	...
	public void genCheckNewInitArraySize(Temp size) {
		Label exit = Label.createLabel();
		Temp cond = genLes(size, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg = genLoadStrConst(RuntimeError.NEGATIVE_ARR_INIT_SIZE);
		genParm(msg);
		genIntrinsicCall(Intrinsic.PRINT_STRING);
		genIntrinsicCall(Intrinsic.HALT);
		genMark(exit);
	...
	public Temp genNewInitArray(Temp length,Temp val) {
		genCheckNewInitArraySize(length);
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp size = genAdd(unit, genMul(unit, length));
		genParm(size);
		Temp obj = genIntrinsicCall(Intrinsic.ALLOCATE);
		...
		Label loop = Label.createLabel();
		Label exit = Label.createLabel();
		Temp zero = genLoadImm4(OffsetCounter.WORD_SIZE);
		genAssign(zero,val);
		append(Tac.genAdd(obj, obj, size));
		genMark(loop);
		append(Tac.genSub(size, size, unit));
	...
	public Temp genNewInitClassArray(Temp length,Tree.Expr left) {
		genCheckNewInitArraySize(length);
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp size = genAdd(unit, genMul(unit, length));
		genParm(size);
		...
		int n = ((ClassType)left.type).getSymbol().getSize();
		Temp size1 =  genLoadImm4(n);
        genParm(size1);
        Temp result1 = genIntrinsicCall(Intrinsic.ALLOCATE);
        int time = n / 4 - 1;
        for (int i = 0; i < time; i++)
        {
            Temp tmp = genLoad(left.val, (i+1)  * 4);
            genStore(tmp, result1, (i+1)  * 4);
        }
        genStore(genLoadVTable(((ClassType)left.type).getSymbol().getVtable()), result1, 0);
        genAssign(zero,result1);
